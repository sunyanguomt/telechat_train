...
def save_state_dict_async_plan_patched(
    ...
    if validated_cache_reuse and cached_central_plan:
        ...
        if not loaded_all_plans or not global_md_verify_reuse:
            # MUSA DEBUG
            #2025_1224 gather -> all_gather
            logger.debug("MUSA DEBUG: state dict saver patch 1 works! change gather in save_state_dict_async_plan to all gahter")
...
def save_state_dict_async_finalize_patched(
    ...
    gather_start = time()
    # 2025_1224 gather -> all_gather
    logger.debug("MUSA DEBUG: state dict saver patch 1 works! change gather in save_state_dict_async_finalize to all_gahter")
...
state_dict_saver.save_state_dict_async_plan = save_state_dict_async_plan_patched
state_dict_saver.save_state_dict_async_finalize = save_state_dict_async_finalize_patched

import sys
sys.modules['megatron.core.dist_checkpointing.strategies.state_dict_saver'].save_state_dict_async_plan = save_state_dict_async_plan_patched
sys.modules['megatron.core.dist_checkpointing.strategies.state_dict_saver'].save_state_dict_async_finalize = save_state_dict_async_finalize_patched
